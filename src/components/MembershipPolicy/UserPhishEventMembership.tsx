...
type UserPhishEventMembershipProps = {
...
const UserPhishEventMembership: FC<UserPhishEventMembershipProps> = ({
  ...
  const getElement = (fieldName: string) => {
    if (fieldName === "phishEvent") {
      return getSelectElementOptions("Phish Event");
    }
  ...
  const getRadioOptions = () =>
    timeFrame.map((time) => (
      <Radio.Button
        value={time.key}
        className="flex-1 text-center custom-radio"
      >
        {time.name}
      </Radio.Button>
    ));
        ...
        <Row gutter={[30, 10]}>
          <Col xs={24} lg={12}>
            {getFormField({
              label: "Phish Event",
              name: "phishEvent",
              errorMsg: "Please select phishEvent",
              type: "select",
              required: true,
            })}
          </Col>
...
export default UserPhishEventMembership;
